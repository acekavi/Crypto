...
use bot::pairs_backtest::{
    ...
    render_promotion_gate_text, render_walk_forward_audit_text, run_backtest, split_summary,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if audit_walk_forward {
        ...
        if json {
            ...
            println!(
                "pair={} risk_sized trades={} win_rate={:.2}% pf={:.3} net={:.4} dd={:.2}% | fixed_{} full_pf={:.3} full_net={:.4} hold_pf={:.3} hold_net={:.4}",
                full.pair,
                full.trades,
                full.win_rate * 100.0,
                full.profit_factor,
