// DIAGNOSTIC — same entry/stop grid as diag_level_entry_stop, with the NY
// session gate turned on. Everything else held identical so this isolates
// the session filter's effect and nothing else.
...
async fn diag_level_entry_stop_ny() {
    ...
    println!(
        "DIAG same grid, session_filter=true (NY 13:30-20:00 UTC), 0.25% risk, react=Reversal, level_tf=H4\n"
    );
    println!(
