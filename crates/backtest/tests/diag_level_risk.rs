// DIAGNOSTIC — is the edge a property of the signal, or of which trades the
// margin budget happened to fund?
...
async fn diag_level_risk() {
    ...
    println!(
        "DIAG {:<10}{:>8}{:>9}{:>10}{:>9}{:>12}",
        "risk%", "n", "win%", "PF", "maxDD", "net"
    );
    for risk in ["0.0025", "0.005", "0.0075", "0.01", "0.02"] {
        let p = LevelParams {
            react_mode: ReactMode::Both,
        ...
        let pct: f64 = risk.parse::<f64>().unwrap() * 100.0;
        println!(
            "DIAG {:<10.2}{:>8}{:>9}{:>10}{:>8.1}%{:>12}",
            pct,
            m.trade_count,
